 ...
 * @module parse/named
 *
 * CSS named color parsing with case-insensitive lookup.
 * Supports all 148 CSS4 named colors plus 'transparent'.
 *
 * @example
 * ```typescript
 * import { parseNamed, tryParseNamed } from 'use-color';
 *
 ...
 * if (!result.ok) {
 *   console.log(result.error.code); // 'INVALID_NAMED'
 * }
 * ```
 */
 ...
 * All 148 standard named colors plus 'transparent'.
 * Keys are lowercase for case-insensitive lookup.
 *
 * @see https://developer.mozilla.org/en-US/docs/Web/CSS/named-color
 */
export const NAMED_COLORS: Readonly<Record<string, readonly [number, number, number, number]>> = {
  // Special
  transparent: [0, 0, 0, 0],
 ...
 * Throws ColorParseError for unknown color names.
 *
 * @param str - The color name to parse (case-insensitive)
 * @returns RGBA object with r, g, b (0-255) and a (0-1)
 * @throws {ColorParseError} When the color name is not recognized
 *
 * @example
 * ```typescript
 * const red = parseNamed('red');
 * // { r: 255, g: 0, b: 0, a: 1 }
 *
 * const transparent = parseNamed('transparent');
 * // { r: 0, g: 0, b: 0, a: 0 }
 *
 * parseNamed('notacolor'); // throws ColorParseError
 * ```
 */
...
/**
 * Safely parses a CSS named color string to RGBA values.
 ...
 * if (result.ok) {
 *   console.log(result.value); // { r: 0, g: 0, b: 255, a: 1 }
 * }
 *
 * const invalid = tryParseNamed('notacolor');
 * if (!invalid.ok) {
 *   console.log(invalid.error.code); // 'INVALID_NAMED'
 * }
 * ```
 */
 ...
 * @returns true if the string is a valid named color, false otherwise
 *
 * @example
 * ```typescript
 * isNamedColor('red');     // true
 * isNamedColor('RED');     // true
 * isNamedColor('invalid'); // false
 * ```
 */
